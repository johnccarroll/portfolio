...
const StyledProjectsGridItem = styled(Grid)(({theme}) => ({
    ...
    "&:hover": {
        transform: "scale(1.02)", transition: "transform 150ms ease-in-out 0s", [theme.breakpoints.up("md")]: {
            "& div": {
                opacity: "1",
            ...
            opacity: "0.9 !important", content: "''", position: "absolute", inset: "0px",

            background: "linear-gradient(rgba(0, 0, 0, 0.1) 10%, rgba(0, 0, 0, 0.78) 70%)",
        },
    },
    [theme.breakpoints.down("md")]: {
        maxWidth: "500px", "&::after": {
            opacity: "0.9 !important", content: "''", position: "absolute", inset: "0px",
...
const StyledProjectsImg = styled("img")(({theme}) => ({
    transform: "scale(1.01)", height: "100%", width: "100%", position: "absolute", objectFit: "cover",
}));
...
const StyledProjectsSkillContainer = styled("div")(({theme}) => ({
    marginTop: "0.5rem", display: "flex", flexWrap: "wrap", rowGap: "0.4rem",
}));
...
const StyledProjectsGHLink = styled("a")({
    color: "inherit", "& svg": {
        padding: "0", minWidth: "0", marginTop: "0.43rem", transform: "scale(0.90)", transition: "transform 200ms",
    }, "&:hover": {
        "& svg": {
            transform: "scale(1.15)",
        },
...
const Projects = () => {
    ...
    return (<Scroll.Element name="Projects">
            <StyledGenericRoot ref={projectsContainer}>
                <StyledGenericContainer
                    sx={projectsContainerInView ? {visibility: "visible"} : {visibility: "hidden"}}
                ...
                <StyledProjectsContainer>
                    <StyledProjectsGrid container rowSpacing={6} columnSpacing={12}>
                        {projects.map((project) => (<InView key={project.id} threshold={0.9} triggerOnce={true}>
                                {({ref, inView}) => (<StyledProjectsGridItem
                                        item
                                        xs={12}
                                        sm={5.5}
                                        ref={ref}
                                        inView={inView}
                                        sx={inView ? {
                                            visibility: "visible", animation: "fadeInUp", animationDuration: "1s",
                                        } : {visibility: "hidden"}}
                                    >
                                        <StyledProjectsImg
                                        ...
                                        <StyledProjectsTextField>
                                            ...
                                            {project.websiteLink && (<StyledProjectsOpenLink
                                                    href={project.websiteLink}
                                                    target="_blank"
                                                >
                                                    <LaunchIcon/>
                                                </StyledProjectsOpenLink>)}
                                            {project.githubLink && (<StyledProjectsGHLink
