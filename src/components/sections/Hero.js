...
const StyledHeroGrid = styled(Grid)(({ismobile}) => ({
    alignItems: "center", justifyContent: "space-around", width: "auto", marginBottom: ismobile ? "3.5rem" : "0rem",
}));
...
const StyledHeroSubText = styled(Typography)(({theme}) => ({
    color: theme.palette.textSecondary.main + " !important", fontSize: "clamp(20px, 2vw, 26px) !important",
}));

const ScrollerIcon = styled("div")(({theme}) => ({
    "@keyframes wheel": {
        to: {
            opacity: 0, top: "2.4rem",
        },
    },
    width: "1.5rem",
    height: "3rem",
...
const StyledSeparator = styled("div")(({theme}) => ({
    height: "2px", width: "50%", // Adjust the width as needed
    backgroundColor: theme.palette.textMain.main, margin: "10px auto", // Adjust for vertical spacing and auto for horizontal centering
}));
            ...
            <StyledHeroContainer ismobile={isMobile}>
                <StyledHeroGrid ismobile={isMobile} container spacing={0}>
                    <Grid item style={{paddingLeft: "0 !important"}}>
                        <StyledHeroImage alt="Image of John" src={img}/>
                    </Grid>
                    <StyledHeroText item>
